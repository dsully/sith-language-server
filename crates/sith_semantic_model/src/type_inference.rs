...
use sith_python_ast::{self as ast, AnyNodeRef, Expr, Number, Operator, UnaryOp};
...
impl ClassType {
    ...
    pub fn class_bases(&self, db: &SymbolTableDb) -> Vec<ResolvedType> {
        ...
        match node_with_parent.node() {
            AnyNodeRef::StmtClassDef(ast::ClassDefStmt {
...
impl<'db> TypeInferer<'db> {
    ...
    pub fn infer_node(&mut self, node: &NodeWithParent, nodes: &Nodes) -> ResolvedType {
        match node.node() {
            AnyNodeRef::StmtAssign(ast::AssignStmt { targets, value, .. }) => {
            ...
            AnyNodeRef::StmtAnnAssign(ast::AnnAssignStmt {
                ...
                if self.path.ends_with("stdlib/typing.pyi")
                    && matches!(annotation.as_ref(), Expr::Name(ast::NameExpr {id, ..}) if id == "_SpecialForm")
                {
                    return self.handle_typing_stub_assignment_or_annotation(**node);
                }
            ...
            AnyNodeRef::ExprStmt(ast::ExprStmt { value, .. }) => {
                self.infer_expr(value.as_ref(), nodes)
            }
            AnyNodeRef::Comprehension(ast::Comprehension { iter, .. }) => {
                self.infer_expr(iter, nodes)
            }
            AnyNodeRef::Parameter(ast::Parameter { name, .. })
    ...
    fn handle_typing_stub_assignment_or_annotation(&mut self, node: AnyNodeRef) -> ResolvedType {
        match node {
            AnyNodeRef::StmtAssign(ast::AssignStmt { targets, .. }) => {
                let [target, ..] = targets.as_slice() else {
                    unreachable!()
                };
            ...
            AnyNodeRef::StmtAnnAssign(ast::AnnAssignStmt { target, range, .. }) => {
    ...
    pub fn infer_expr<'a>(
        ...
        let expr = expr.into();
        match expr {
            AnyNodeRef::NameExpr(ast::NameExpr { id, range, .. }) => self.infer_symbol(
                id,
                nodes,
                DeclarationQuery::AtOffset(range.start().to_u32()),
            ),
            ...
            AnyNodeRef::NamedExpr(ast::NamedExpr { value, .. }) => {
                self.infer_expr(value.as_ref(), nodes)
            }
            AnyNodeRef::IfExpr(ast::IfExpr { body, orelse, .. }) => {
                let body = self.infer_expr(body.as_ref(), nodes);
                let orelse = self.infer_expr(orelse.as_ref(), nodes);

                body.union(orelse)
            }
            AnyNodeRef::BoolOpExpr(ast::BoolOpExpr { values, .. }) => values
                .iter()
            ...
            AnyNodeRef::BinOpExpr(ast::BinOpExpr {
                left, op, right, ..
            ...
            AnyNodeRef::SubscriptExpr(ast::SubscriptExpr { value, slice, .. }) => {
                ...
                let slice_type = if matches!(slice.as_ref(), Expr::Name(ast::NameExpr {id, ..}) if id.is_empty())
                {
    ...
    fn resolve_inner_collection_type(&mut self, expr: &Expr, nodes: &Nodes) -> Vec<ResolvedType> {
        ...
        if let Expr::Tuple(ast::TupleExpr { elts, .. }) = expr {
            for element in elts {
    ...
    fn resolve_declaration_type(
        ...
        match declaration_node.node() {
            ...
            AnyNodeRef::StmtFunctionDef(ast::FunctionDefStmt { body, returns, .. }) => {
            ...
            AnyNodeRef::Parameter(ast::Parameter { annotation, .. }) => {
                ...
                if let Some(AnyNodeRef::ParameterWithDefault(ast::ParameterWithDefault {
    ...
    fn infer_pattern_match_as_type(
        ...
        match parent_node.node() {
            AnyNodeRef::StmtMatch(ast::MatchStmt { subject, .. }) => {
            ...
            AnyNodeRef::PatternMatchSequence(ast::PatternMatchSequence { .. }) => {
...
fn is_collection_typing(expr: &Expr) -> bool {
    let Expr::Name(ast::NameExpr { id, .. }) = expr else {
        return false;
    };
...
mod tests {
    ...
    use sith_python_utils::{interpreter::resolve_python_interpreter, PythonHost};
    ...
    fn assert_type(src: &str, path: impl AsRef<Path>, expected: &str) {
        ...
        let reveal_type_func = stack.nodes().iter().find(|node_with_parent| {
            matches!(
                ...
                AnyNodeRef::CallExpr(ast::CallExpr { func, .. })
                    if matches!(func.as_ref(), Expr::Name(ast::NameExpr {id, ..}) if id == "reveal_type")
            )
