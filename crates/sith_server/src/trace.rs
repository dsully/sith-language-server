//! The tracing system for SithLSP.
//!
...
//! also be set with `sith.trace.server`. A trace level of `messages` or
...
//!
//! The server also supports the `SITH_TRACE` environment variable, which will
...
const TRACE_ENV_KEY: &str = "SITH_TRACE";
...
impl<S> tracing_subscriber::layer::Filter<S> for LogLevelFilter {
    fn enabled(
        ...
        let filter = if meta.target().starts_with("sith") {
