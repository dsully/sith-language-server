...
pub fn call_expr_identifier(expr: &Expr) -> Option<&str> {
    match expr {
        Expr::Name(python_ast::NameExpr { id, .. }) => Some(id.as_str()),
        Expr::Call(python_ast::CallExpr { func, .. }) => call_expr_identifier(func),
        Expr::Attribute(python_ast::AttributeExpr { attr, .. }) => Some(attr.as_str()),
        _ => None,
    }
}
...
mod tests {
    use super::*;
    use python_ast::Expr;
    use python_parser::{parse_unchecked, Mode};
